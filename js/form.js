  ...
  checkValidationLayerOne() {
    this.#createAccountForm.addEventListener("submit", (e) => {
      ...
      this.#gender = [
        ...
        .map((gender) => (gender.checked ? gender.value : null))
        .filter((gender) => gender !== null)[0];
      if (
        this.#username.value &&
        this.#email.value &&
        ...
        if (
          ...
          this.#dateOfBirth.value === undefined
        )
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Date of Birth, Cannot be empty"
          );
        if (
          this.#phone.value === "" ||
          ...
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Phone Number, Cannot be empty"
          );
        if (
          this.#confirmPassword.value === "" ||
          this.#confirmPassword.value === null ||
          this.#confirmPassword.value === undefined
        )
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Confirm Password, Cannot be empty"
          );
        if (
          this.#CreateAccountpassword.value === "" ||
          this.#CreateAccountpassword.value === null ||
          this.#CreateAccountpassword.value === undefined
        )
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Password, Cannot be empty"
          );
        if (
          this.#email.value === "" ||
          ...
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Email, Cannot be empty"
          );
        if (
          this.#username.value === "" ||
          ...
          this.#renderMessageError(
            this.#messageSignUp,
            "Check your Username, Cannot be empty"
          );
        flagOne = false;
      }
    });
  ...
  LoginValidation() {
    this.#signInForm.addEventListener("submit", (e) => {
      ...
      if (
        ...
        this.#signInEmail.value === null ||
        this.#signInEmail.value === undefined
      )
        this.#renderMessageError(
          this.#messageSignIn,
          "Check your Email, Cannot be empty"
      ...
      if (
        emailRegex.test(this.#signInEmail.value) &&
        passwordRegex.test(this.#password.value)
      ) {
    ...
    this.#createAccountForm.addEventListener("submit", (e) => {
      ...
      if (this.checkValidationLayerOne()) {
        ...
        if (!passwordRegex.test(this.#CreateAccountpassword.value))
          this.#renderMessageError(
            this.#messageSignUp,
            "Your password is not valid , it must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, and one number"
          );
